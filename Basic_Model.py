...
eval_env = make_env(num_envs, num_levels=num_levels, env_name='starpilot')
...
nombre = 'expR'+str(fecha.month)+'_'+ str(fecha.day)+'_'+str(fecha.hour)+str(fecha.minute)
...
step = 0
aux = 0
with open("results_trainingR.txt", "w") as f:
	while step < total_steps:
		# Define environment
		# check the utils.py file for info on arguments
		env = make_env(num_envs, num_levels=num_levels, env_name='starpilot', seed = random.randint(0,2**31-1))
		storage.reset()
...
torch.save(policy.state_dict, 'checkpointR.pt')

import pickle

with open ('modelR.pkl', 'wb') as f2:
...
imageio.mimsave('vid512R.gif', frames, fps=25)
...
# Save frames as video
frames = torch.stack(frames)
imageio.mimsave('vid1024R.gif', frames, fps=25)
