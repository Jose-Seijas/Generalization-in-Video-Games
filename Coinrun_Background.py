...
class Policy(nn.Module):
  def __init__(self, encoder, feature_dim, num_actions):
    ...
    self.policy = orthogonal_init(nn.Linear(feature_dim, num_actions), gain=.005)
...
env = make_env(num_envs, num_levels=num_levels, use_backgrounds=True, env_name='coinrun')
print('Observation space:', env.observation_space)
print('Action space:', env.action_space.n)
eval_env = make_env(num_envs, num_levels=num_levels, use_backgrounds=True, env_name='coinrun')
...
nombre = 'expRBG_CR'+str(fecha.month)+'_'+ str(fecha.day)+'_'+str(fecha.hour)+str(fecha.minute)
...
step = 0
aux = 0
with open("results_trainingRBG_CR.txt", "w") as f:
	while step < total_steps:
		# Define environment
		# check the utils.py file for info on arguments
		env = make_env(num_envs, num_levels=num_levels, use_backgrounds=True, env_name='coinrun', seed = random.randint(0,2**31-1))
		storage.reset()
		...
		if aux == 50:
			...
			print(f'{step}\t{storage.get_reward()}\t{torch.std(std_train.sum(0))}t{storage_test.get_reward()}\t{torch.std(std_test.sum(0))}', file=f)
...
torch.save(policy.state_dict, 'checkpointRBG_CR.pt')

import pickle

with open ('modelRBG_CR.pkl', 'wb') as f2:
...
imageio.mimsave('vid512RBG_CR.gif', frames, fps=25)
...
# Save frames as video
frames = torch.stack(frames)
imageio.mimsave('vid1024RBG_CR.gif', frames, fps=25)
